...
import (
	...
	"github.com/mrzdtydlntm/golang_backend/helper"
...
func (c *bookController) FindByID(context *gin.Context) {
	id, err := strconv.ParseUint(context.Param("id"), 0, 0) //kita ambil params dari url nya
	if err != nil {
...
func (c *bookController) Insert(context *gin.Context) {
	...
	if errDTO != nil {
		...
		userID := c.getUserIDByToken(authHeader) //ini string jadi harus di convert lg
...
func (c *bookController) Update(context *gin.Context) {
	...
	if c.bookService.IsAllowedToEdit(userID, bookUpdateDTO.ID) {
		...
		response := helper.BuildErrorResponse("You don't have permission", "You're not the owner", helper.EmptyObj{})
